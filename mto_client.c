...
int parse_cmd(int argc, char * const argv[], char **server_name , int *my_num);
...
int main(int argc, char **argv)
{
	...
	status = parse_cmd(argc, argv, &client_target_name , &my_num);
  ...
  CHKERR_JUMP(status != UCS_OK, "ucp_worker_create\n", err_cleanup);
        ...
        printf("my port is: %d\n", server_port + my_num);
        oob_sock = client_connect(client_target_name, server_port + my_num);
    ...
    while ((c = getopt(argc, argv, "wfben:c:p:s:h")) != -1) {
        ...
        default:
            ...
            fprintf(stderr, "  -n name Set node name or IP address "
                    ...
                    "for server)\n");
            fprintf(stderr, "  -c client's serial number(starting from 1 to total clients number)\n");
